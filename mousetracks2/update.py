...
from .utils.network import safe_download_file
...
def generate_exe_name(version: str) -> str:
...
def cleanup_old_executables(folder: Path | str, version: str = VERSION, keep: int = 2) -> None:
    ...
    # Trim off the latest versions if required
    if keep:
        lower = lower[:-keep]
